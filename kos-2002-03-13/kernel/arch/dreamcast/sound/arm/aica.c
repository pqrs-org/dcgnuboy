...
extern volatile aica_channel *chans;
...
void aica_play(int ch) {
	unsigned long smpptr	= chans[ch].pos;
...
void aica_vol(int ch) {
	CHNREG8(ch, 41) = AICA_VOL(chans[ch].vol);
}

/* Set channel pan */
void aica_pan(int ch) {
	CHNREG8(ch, 36) = AICA_PAN(chans[ch].pan);
}
