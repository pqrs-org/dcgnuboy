...
CVSID("snd_sfxmgr.c,v 1.1 2002/02/10 06:40:06 bardtx Exp");
...
int snd_sfx_load(const char *fn) {
	...
	fs_read(fd, tmp, len);
	fs_close(fd);
	sfx[idx].loc = snd_stream_load_sample(tmp, len);
	free(tmp);
...
void snd_sfx_play(int idx, int vol, int pan) {
	...
	snd_stream_play_effect(sfx_lastchan, sfx[idx].loc, sfx[idx].rate, size, vol, pan);
