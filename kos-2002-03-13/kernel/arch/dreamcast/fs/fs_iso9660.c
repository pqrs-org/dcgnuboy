...
CVSID("fs_iso9660.c,v 1.3 2002/02/13 05:28:29 bardtx Exp");
...
static spinlock_t cache_mutex;

/* Clears all cache blocks */
static void bclear_cache(cache_block_t **cache) {
	int i;
	
	spinlock_lock(&cache_mutex);
	for (i=0; i<NUM_CACHE_BLOCKS; i++)
		cache[i]->sector = -1;
	spinlock_unlock(&cache_mutex);
}
...
static iso_dirent_t *find_object(const char *fn, int dir,
	...
	while (dir_size > 0) {
		...
		for (i=0; i<2048 && i<dir_size; ) {
			...
			if (rrnamelen > 0) {
				char *p = strchr(fn, '/');
				int is_match_filename;
				
				if (p) {
					is_match_filename = 
						!strnicmp(rrname, fn, p - fn);
				} else {
					is_match_filename = 
						!stricmp(rrname, fn);
				}
...
ssize_t iso_read(uint32 fd, void *buf, size_t bytes) {
	...
	while (bytes > 0) {
		...
		if (c < 0) return -1;
		memcpy(buf, dcache[c]->data + (fh[fd].ptr%2048), toread);
...
int iso_ioctl(uint32 hnd, void *data, size_t size) {
	int i;
