...
typedef struct {
	...
	/* Running statistics on the PVR system; most of these aren't implemented yet */
	uint32		frame_count;			/* Frame counter for animations and such */
	float		frame_last_time;		/* Render length for the last frame */
	uint32		frame_last_sec,			/* When did the last frame finish? */
			frame_last_msec;
} pvr_state_t;
