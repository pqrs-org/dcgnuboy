...
CVSID("pvr_scene.c,v 1.4 2002/02/04 07:51:40 bardtx Exp");
...
int pvr_list_begin(pvr_list_t list) {
	...
	if (pvr_state.opb_open_list != -1) {
		dbglog(DBG_WARNING, "pvr_list_begin: attempt to open a list when one is already open\n");
		return -1;
	}
#endif	/* !NDEBUG */
...
int pvr_wait_ready() {
	...
	ticks = jiffies + 10;
	while (pvr_state.flip_completed == 0 && (jiffies < ticks)) {
		if (thd_enabled)
			thd_pass();
	}
	if (jiffies >= ticks) {
