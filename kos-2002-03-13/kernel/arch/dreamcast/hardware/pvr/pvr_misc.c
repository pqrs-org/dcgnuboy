...
CVSID("pvr_misc.c,v 1.9 2002/02/24 00:21:56 bardtx Exp");
...
int pvr_get_frame_count() {
	return pvr_state.frame_count;
}
...
int pvr_get_stats(pvr_stats_t *stat) {
	...
	stat->frame_last_time = pvr_state.frame_last_time;
	if (stat->frame_last_time != 0)
		stat->frame_rate = 1.0f / stat->frame_last_time;
	else
...
/* Update statistical counters (called once per frame completion IRQ) */
void pvr_sync_stats() {
	uint32	s, m, ds, dm;
	...
	/* Is this the first frame ever? If so, don't bother doing any
	   stats right now... */
	if (pvr_state.frame_count == 0) {
		pvr_state.frame_last_sec = s;
		pvr_state.frame_last_msec = m;
	...
	ds = s - pvr_state.frame_last_sec;
	dm = m - pvr_state.frame_last_msec;
	pvr_state.frame_last_time = (dm / 1000.0) + ds;
	pvr_state.frame_last_sec = s;
	pvr_state.frame_last_msec = m;
}
