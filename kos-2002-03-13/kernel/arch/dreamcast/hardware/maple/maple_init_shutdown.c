...
CVSID("maple_init_shutdown.c,v 1.2 2002/03/04 04:37:10 bardtx Exp");
...
int maple_hw_init() {
	...
	asic_evt_set_handler(ASIC_EVT_PVR_VBLINT, maple_vbl_irq_hnd);	asic_evt_enable(ASIC_EVT_PVR_VBLINT);
	asic_evt_set_handler(ASIC_EVT_MAPLE_DMA, maple_dma_irq_hnd);	asic_evt_enable(ASIC_EVT_MAPLE_DMA);
...
void maple_hw_shutdown() {
	...
	/* Unhook interrupts */
	asic_evt_set_handler(ASIC_EVT_PVR_VBLINT, NULL);	asic_evt_disable(ASIC_EVT_PVR_VBLINT);
	asic_evt_set_handler(ASIC_EVT_MAPLE_DMA, NULL);		asic_evt_disable(ASIC_EVT_MAPLE_DMA);
