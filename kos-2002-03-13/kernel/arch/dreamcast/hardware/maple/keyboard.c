...
CVSID("keyboard.c,v 1.3 2002/02/22 08:20:10 bardtx Exp");
...
/* Retrieve the key state structure for the given keyboard */
kbd_state_t * kbd_get_state(int p, int u) {
	maple_device_t	*dev;

	dev = &maple_state.ports[p].units[u];
	if (!dev->valid)
		return NULL;
	if (!(dev->info.functions & MAPLE_FUNC_KEYBOARD))
		return NULL;

	return (kbd_state_t *)dev->status;
}
