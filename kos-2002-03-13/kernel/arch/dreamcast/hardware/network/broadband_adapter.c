...
CVSID("broadband_adapter.c,v 1.6 2002/02/16 06:54:58 bardtx Exp");
...
static void bba_irq_hnd(irq_t src, irq_context_t *context);
...
static int bba_hw_init() {
	...
	g2_write_32(NIC(RT_TXCONFIG), 00000600);
	...
	irq_set_handler(EXC_IRQB, bba_irq_hnd);
	ASIC_IRQB_B = 1 << 3;
...
static void bba_hw_shutdown() {
	...
	ASIC_IRQB_B &= ~(ASIC_EVT_EXP_PCI & 0xff);
...
static int bba_if_detect(struct netif *self) {
...
static int bba_if_init(struct netif *self) {
...
static int bba_if_shutdown(struct netif *self) {
	if (!(bba_if.flags & NETIF_INITIALIZED))
		return 0;
...
static int bba_if_start(struct netif *self) {
	if (!(bba_if.flags & NETIF_INITIALIZED))
		return -1;
...
static int bba_if_stop(struct netif *self) {
...
static int bba_if_tx(struct netif *self, const uint8 *data, int len, int blocking) {
	if (!(bba_if.flags & NETIF_RUNNING))
		return -1;
...
static int bba_if_tx_commit(struct netif *self) {
	return 0;
}

/* All RX is done via the interrupt */
static int bba_if_rx_poll(struct netif *self) {
	return -1;
}

/* Don't need to hook anything here yet */
static int bba_if_set_flags(struct netif *self, uint32 flags_and, uint32 flags_or) {
...
int bba_init() {
	...
	/* Setup the structure */
	bba_if.name = "bba";
	bba_if.descr = "Broadband Adapter";
	bba_if.index = 0;
	bba_if.dev_id = 0;
