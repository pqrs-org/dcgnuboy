...
CVSID("lan_adapter.c,v 1.6 2002/02/16 06:55:04 bardtx Exp");
...
/* Forward declaration */
static void la_irq_hnd(irq_t src, irq_context_t *context);
...
static int la_hw_init() {
	...
	/* Setup interrupt handler: like in BBA */
	irq_set_handler(EXC_IRQB, la_irq_hnd);
	ASIC_IRQB_B = 1 << 2;
...
static int la_if_detect(struct netif * self) {
...
static int la_if_init(struct netif * self) {
...
static int la_if_shutdown(struct netif * self) {
	if (!(self->flags & NETIF_INITIALIZED))
		return 0;
...
static int la_if_start(struct netif * self) {
	if (!(self->flags & NETIF_INITIALIZED))
		return -1;
...
static int la_if_stop(struct netif * self) {
...
static int la_if_tx(struct netif * self, const uint8 * data, int len, int blocking) {
	if (!(self->flags & NETIF_RUNNING))
		return -1;
...
static int la_if_tx_commit(struct netif * self) {
	return 0;
}

static int la_if_rx_poll(struct netif * self) {
	return la_rx();
}

/* Don't need to hook anything here yet */
static int la_if_set_flags(struct netif * self, uint32 flags_and, uint32 flags_or) {
