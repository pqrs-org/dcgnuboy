...
CVSID("irq.c,v 1.4 2002/02/18 03:04:43 bardtx Exp");
...
void irq_handle_exception(int code) {
	...
	/* Re-enable exceptions so we can get errors */
	irq_enable_exc();
	...
	/* Debug code to let us catch TRAPA */
	/* if (evt == EXC_TRAPA && (*tra & (0xff<<2)) == 0) {
		extern int thd_enabled;
		uint32 *regs = (uint32*)srt_addr;
		thd_enabled = 0;
		printf("TRAPA handler entered (from %08x, SR=%08x, PR=%08x)\n", regs[16], regs[22], regs[17]);
		thd_enabled = 1;
	} */
...
int irq_init() {
	...
	asm("	! Set VBR\n"
	    ...
	    "	.align 2\n"
	"_vbr_addr:\n"
	    "	.long	_irq_vma_table		! Non-cached area\n"
	"_after_vbr:\n");
