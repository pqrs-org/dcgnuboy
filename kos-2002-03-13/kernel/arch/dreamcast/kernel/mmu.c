...
CVSID("mmu.c,v 1.2 2002/01/06 01:14:48 bardtx Exp");
...
int mmu_copyv(mmucontext_t *context1, iovec_t *iov1, int iovcnt1,
	...
	while (srciov < iovcnt1 && dstiov < iovcnt2) {
		...
		/* Do the segment copy */
		if (!sproket) {
			dbgio_printf("Copying %08x -> %08x (%08x -> %08x), %d bytes\n",
...
static void unhandled_mmu(irq_t source, irq_context_t *context) {
	...
	dbgio_printf("Exception happened in tid %d at PC %08x, SR %08x\n",
		thd_current->tid, context->pc, context->sr);
	dbgio_printf(" PTEH = %08x, PTEL = %08x\n", *pteh, *ptel);
	dbgio_printf(" TTB = %08x, TEA = %08x\n", *ttb, *tea);
	dbgio_printf(" MMUCR = %08x\n", *mmucr);
...
static void gen_tlb_miss(const char *what, irq_t source, irq_context_t *context) {
	...
	page =  map_virt(mmu_cxt_current, addr >> PAGESIZE_BITS);
	if (!page) {
		dbgio_printf("%s: cannot map virtual address %08x\n", what, addr);
