/* KallistiOS ##version##
   ...
   net.h,v 1.5 2002/02/16 06:51:30 bardtx Exp
...
typedef struct netif {
	...
	int (*if_detect)(struct netif * self);

	/* Initialize the device */
	int (*if_init)(struct netif * self);

	/* Shutdown the device */
	int (*if_shutdown)(struct netif * self);

	/* Start the device (after init or stop) */
	int (*if_start)(struct netif * self);

	/* Stop (hibernate) the device */
	int (*if_stop)(struct netif * self);

	/* Queue a packet for transmission; see special return values
	   below the structure */
	int (*if_tx)(struct netif * self, const uint8 * data, int len, int blocking);

	/* Commit any queued output packets */
	int (*if_tx_commit)(struct netif * self);

	/* Poll for queued receive packets, if neccessary */
	int (*if_rx_poll)(struct netif * self);

	/* Set flags; you should generally manipulate flags through here so that
	   the driver gets a chance to act on the info. */
	int (*if_set_flags)(struct netif * self, uint32 flags_and, uint32 flags_or);
} netif_t;
